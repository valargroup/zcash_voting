...
// The signature covers sighash = Blake2b-256("ZALLY_SIGHASH_V0"), which must
// match the placeholder in api/codec.go ComputeSigHash.
//
// CRITICAL: When ComputeSigHash is updated to a proper sig-excluding hash,
// these fixtures must be regenerated to match the new sighash derivation.
...
/** Real RedPallas signature over the placeholder sighash, 64 bytes, base64-encoded. */
...
/**
 * Build a valid MsgRegisterDelegation JSON body with a real RedPallas
 * signature (verified by the Rust FFI verifier when chain is built with
 * `-tags redpallas`).
 *
 * The `rk` and `spend_auth_sig` come from pre-computed RedPallas fixtures
 * that sign the deterministic placeholder sighash. The ZKP proof uses real
 * Halo2 data when fixture files are available, otherwise falls back to mock
 * data (passes MockVerifier or when chain is built without `-tags halo2`).
 *
