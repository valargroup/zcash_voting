...
pub fn generate_orchard_witnesses_at_frontier(
...
) -> Result<Vec<incrementalmerkletree::MerklePath<orchard::tree::MerkleHashOrchard, { orchard::NOTE_COMMITMENT_TREE_DEPTH as u8 }>>, SqliteClientError> {
    ...
    let mem_conn = rusqlite::Connection::open_in_memory()
        .map_err(|e| SqliteClientError::DbError(e))?;
    ...
    let store = SqliteShardStore::<_, orchard::tree::MerkleHashOrchard, ORCHARD_SHARD_HEIGHT>::from_connection(
        &tx, "orchard",
    ).map_err(|e| SqliteClientError::DbError(e))?;
    ...
    .map_err(|e| SqliteClientError::CorruptedData(
        format!("failed to insert frontier nodes: {}", e),
    ))?;

    tree.store_mut()
        .add_checkpoint(checkpoint_height, Checkpoint::at_position(frontier_position))
        .map_err(|e| SqliteClientError::CorruptedData(
            format!("failed to add checkpoint: {}", e),
        ))?;
